...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Marcin\\Desktop\\simpleConjunction\\conSearch3_lastrun.py',
...
targetRedHor = visual.Rect(
    win=win, name='targetRedHor',units='cm', 
...
trial1Components = [targetRedHor, dist1, dist2, dist3, dist4, dist5, dist6, dist7, dist8, dist9, dist10, dist11, dist12, dist13, dist14, dist15]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    # *targetRedHor* updates
    if targetRedHor.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        targetRedHor.frameNStart = frameN  # exact frame index
        targetRedHor.tStart = t  # local t and not account for scr refresh
        targetRedHor.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(targetRedHor, 'tStartRefresh')  # time at next scr refresh
        targetRedHor.setAutoDraw(True)
    if targetRedHor.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > targetRedHor.tStartRefresh + 1.0-frameTolerance:
            # keep track of stop time/frame for later
            targetRedHor.tStop = t  # not accounting for scr refresh
            targetRedHor.frameNStop = frameN  # exact frame index
            win.timeOnFlip(targetRedHor, 'tStopRefresh')  # time at next scr refresh
            targetRedHor.setAutoDraw(False)
...
thisExp.addData('targetRedHor.started', targetRedHor.tStartRefresh)
thisExp.addData('targetRedHor.stopped', targetRedHor.tStopRefresh)
