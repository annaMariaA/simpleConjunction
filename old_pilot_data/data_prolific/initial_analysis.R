...
ggplot(summarydata, aes(x=difficulty, y = z_score, group=targPresent, shape=targPresent, colour=targPresent)) +
  geom_line(size=1)+
  geom_errorbar(aes(ymin=z_score-ci, ymax=z_score+ci), width=.2,
                position=position_dodge(.1))+
  facet_wrap(~targetType)+labs(color="target status", x="difficulty", y="rt(s)")->rt_slope_2#+ theme(legend.position=c(0.88,0.19))
