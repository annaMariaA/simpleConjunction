...
rate_slopes <- c(
  paste("targ", c("absent", "present"), ":typeredHorizontal", ":nD", sep = ""),
  paste("targ", c("absent", "present"), ":typegreenVertical", ":nD", sep = ""))

b_slopes <- paste("type", unique(d$type), ":nD", sep = "")
...
formula <- bf(rt | dec(response) ~ 
                ...
                0 + targ:type  + targ:type:nD  + 
                (0 + targ:type  + targ:type:nD|p|obs), 
              # bs: boundary separation 
              bs ~ 0 + type + type:nD + (0 + type + type:nD|p|obs), 
              # ndt: non-decision time
              ndt ~ 1 + (1|p|obs),
              # bias: starting bias for TA or TP
              bias ~ 0 + type + type:nD + (0 + type + type:nD|p|obs))
...
saveRDS(fit_wiener, "level3_full.model")
...
saveRDS(fit_wiener, "level3_redu.model")
